...
document.addEventListener("DOMContentLoaded", () => {
  const table = $("#travelTable").DataTable({
    processing: true,
    serverSide: true,
    responsive: false,
    scrollX: false,
    ...
    columns: [
  {
    ...
    render: function (data, type, row) {
      ...
      // Helper kecil untuk mengecek apakah nilai akses benar-benar 'Y' / true / 1
      // 1. TAMPILKAN TOMBOL EDIT JIKA 'Y' / true
      if (row.akses?.edit) {
        buttons += `
          <button onclick="editTravel(${row.id})" class="p-2 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-500/10 dark:text-blue-400 transition-colors" title="Edit">
            <i class="ph-bold ph-pencil-simple text-lg"></i>
          </button>`;
      }

      // 2. TAMPILKAN TOMBOL DELETE JIKA 'Y' / true
      if (row.akses?.delete) {
        buttons += `
          <button onclick="deleteTravel(${row.id})" class="p-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-500/10 dark:text-red-400 transition-colors" title="Hapus">
            <i class="ph-bold ph-trash text-lg"></i>
          </button>`;
      }

      // Jika kedua akses N / false, tampilkan strip/spasi
  ...
  },
  {
    data: "fullname",
    title: "fullname",
    className: "font-semibold text-gray-900 dark:text-white p-2 border-b",
  },
  {
    data: "username",
    title: "Username",
    ...
    render: (data) =>
      `<span class="px-2 py-1 rounded-md bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 text-xs font-mono">${data}</span>`,
  },
  {
    data: "email",
    title: "Email",
  ...
  },
  {
    ...
    title: "ID Level",
    className: "p-2 border-b font-semibold text-gray-900 dark:text-white",
  ...
  document.getElementById("submitUserBtn").addEventListener("click", async function (e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          document.getElementById("travelFormModal").classList.add("hidden");
          $("#travelTable").DataTable().ajax.reload();
        }, 1500);
      } else {
...
// Tambahkan Pengecekan null agar aman jika tombol Tambah Travel ter-hide oleh EJS
...
if (addTravelBtn) {
  addTravelBtn.addEventListener("click", (e) => {
    ...
    document.getElementById("travelFormModal").classList.remove("hidden");
...
window.editTravel = async function (id) {
  try {
...
window.deleteTravel = function (id) {
  swal({
